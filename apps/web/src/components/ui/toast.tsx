...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const icons: Record<ToastVariant, React.ReactNode> = {
    default: <Info className="h-4 w-4 text-[hsl(var(--accent-foreground))]" />,
    success: <CheckCircle className="h-4 w-4 text-[hsl(142,71%,45%)]" />,
    error: <AlertCircle className="h-4 w-4 text-[hsl(var(--destructive))]" />,
    warning: <AlertTriangle className="h-4 w-4 text-[hsl(38,92%,50%)]" />,
  }
      ...
      <div className="fixed bottom-4 right-4 z-[100] flex flex-col gap-2 w-80">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={cn(
              'flex items-start gap-3 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4 shadow-xl animate-fade-in',
            )}
          >
